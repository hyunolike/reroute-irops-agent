resource "aws_security_group" "alb" {
  name        = "${var.name}-alb"
  description = "Public entry: HTTP/HTTPS to the ALB only"
  vpc_id      = aws_vpc.main.id

  dynamic "ingress" {
    for_each = local.cloudfront ? [] : [80, 443]
    content {
      description = ingress.value == 80 ? "HTTP" : "HTTPS"
      from_port   = ingress.value
      to_port     = ingress.value
      protocol    = "tcp"
      cidr_blocks = var.allowed_ingress_cidrs
    }
  }
  # With CloudFront only its origin-facing ranges reach the ALB (HTTP; CloudFront terminates HTTPS).
  # The prefix list counts ~55 entries against the 60-rules quota, so it is used for port 80 only.
  dynamic "ingress" {
    for_each = local.cloudfront ? [80] : []
    content {
      description     = "HTTP from CloudFront only"
      from_port       = 80
      to_port         = 80
      protocol        = "tcp"
      prefix_list_ids = [data.aws_ec2_managed_prefix_list.cloudfront[0].id]
    }
  }
  egress {
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
    cidr_blocks = [var.vpc_cidr]
  }
}

resource "aws_security_group" "app" {
  name        = "${var.name}-app"
  description = "App host: reachable only from the ALB; no SSH (use SSM Session Manager)"
  vpc_id      = aws_vpc.main.id

  ingress {
    description     = "web"
    from_port       = 3000
    to_port         = 3000
    protocol        = "tcp"
    security_groups = [aws_security_group.alb.id]
  }
  ingress {
    description     = "api"
    from_port       = 8000
    to_port         = 8000
    protocol        = "tcp"
    security_groups = [aws_security_group.alb.id]
  }
  egress {
    description = "outbound (ECR, Secrets Manager, NVIDIA endpoints) via NAT"
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
    cidr_blocks = ["0.0.0.0/0"]
  }
}

resource "aws_security_group" "db" {
  name        = "${var.name}-db"
  description = "PostgreSQL reachable only from the app host"
  vpc_id      = aws_vpc.main.id

  ingress {
    from_port       = 5432
    to_port         = 5432
    protocol        = "tcp"
    security_groups = [aws_security_group.app.id]
  }
}
