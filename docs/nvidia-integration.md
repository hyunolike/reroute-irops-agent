...
|---|---|---|---|
| **Nemotron via NIM** | goal interpretation, planning, tool selection, operator briefing, exception narrative | `POST https://integrate.api.nvidia.com/v1/chat/completions`, `tools` + `tool_choice: "auto"`, `message.tool_calls`; Nemotron 3: `chat_template_kwargs.enable_thinking`; Llama-Nemotron v1.5: `/think`·`/no_think` | Contract-tested (`test_agent.py`, `test_llm_agent.py`): request shape, text-form tool calls, `<think>` handling, retries, and imperfect-model behaviour (sequential calls, premature calls, early stops). **Verified live** (2026-09-25, hosted `nvidia/nemotron-3-super-120b-a12b`): `make eval-llm` 4/4 in three consecutive runs, no planner fallback; transient 429/5xx absorbed by capped backoff honouring `Retry-After` |
| **NeMo Retriever** | policy search with provenance | `POST /v1/embeddings` (`input_type` passage/query) with `nvidia/nemotron-3-embed-1b`; `POST https://ai.api.nvidia.com/v1/retrieval/nvidia/llama-nemotron-rerank-vl-1b-v2/reranking` → `rankings[{index, logit}]` | **Verified live** (2026-09-25): same top-1 policy as BM25 on the agent's English queries, and finds the right policy for Korean queries where BM25 returns nothing; `make eval-llm` with `RETRIEVER_PROVIDER=nvidia` 4/4 twice, no retriever errors. The earlier `llama-nemotron-embed-1b-v2` / `rerank-1b-v2` were retired on 2026-08-25 (HTTP 410). Lexical fallback verified |
...
| **NemoClaw · OpenClaw** | OpenClaw (in a NemoClaw sandbox) plans and calls ReRoute tools over MCP; human approval stays in ReRoute | `nemoclaw <name> mcp add <server> --url https://… --env TOKEN` (Streamable HTTP, bearer, HTTPS; generated OpenShell `protocol: mcp` policy), `--deny-tool`, `mcp status`, `skill install` | MCP server **Verified** (`test_mcp.py`, real HTTP, MCP SDK 2.x) · NemoClaw registration **Documented** (not executed) |
